...
# base_path = '/data'
base_path = '/media/Data1/BOP/ycb-v/ycbv/'
output_base_path = '/media/Data1/BOP/ycb-v/ycbv/annotations/'
...
if annotation_set == "train":
    if train_set == "full":
        ...
        only_keyframes = False
    elif train_set == "pbr_only_squirrel":
        # Annotate only the images generated by PBR
        data_paths = ['train_pbr/']
        img_types = ['pbr']
        annotation_path = 'train_pbr_only_squirrel.json'
        only_keyframes = False
...
categories = [
    ...
    {'supercategory': '062_squirrel', 'id': 22, 'name': '062_squirrel'},
]
if train_set == "pbr_only_squirrel" or test_set == "bop":
    categories = [{'supercategory': '062_squirrel', 'id': 22, 'name': '062_squirrel'}]

category_ids = [cat['id'] for cat in categories]

for ds, settings in datasets.items():
    data_paths = settings['data_paths']
    annotation_paths = settings['annotation_paths']
    img_types = settings['img_types']
    ...
    for data_path, ann_path, img_type in zip(data_paths, annotation_paths, img_types):
        ...
        for img_dir in image_dirs:
            ...
            img_names.sort()
            with open(img_dir_path + 'scene_gt_coco.json', 'r') as f:
                coco = json.load(f)
            img_width, img_height = coco['images'][0]['width'], coco['images'][0]['height']
            ...
            for img_name, b_k, p_k, c_k in zip(img_names, bbox_annotations, pose_annotations, camera_annotations):
                ...
                for bbox, pose, in zip(bbox_data, pose_data):
                    ...
                    if int(pose['obj_id']) not in category_ids:
                    ...
                    if x2 >= img_width:
                        # Adjust width
                        bbox['bbox_obj'][2] = img_width - bbox['bbox_obj'][0] - 1

                    if y2 >= img_height:
                        # Adjust height
                        bbox['bbox_obj'][3] = img_height - bbox['bbox_obj'][1] - 1
